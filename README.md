...
| Render 免費方案 | 0 元 |
| Notion / Google Calendar / GitHub Actions | 0 元 |
| Claude API | 約 NT$20-40 / 月 |

唯一要花錢的是 Claude API。每次呼叫的固定開銷約 1,400 tokens（system prompt 476 + 工具定義 921），一則需要查資料的對話會打兩次 API（先決定用哪個工具、再看結果回答），合計約 3,400 input + 600 output tokens。

用 `claude-opus-5`（$5 / $25 per MTok）換算一則約 NT$1.0。但規則層會擋掉大部分呼叫（見下節），實際只有少數訊息真的送到 Claude，所以月費落在 NT$20-40。

想再壓低就把 `.env` 的 `CLAUDE_MODEL` 改成 `claude-sonnet-5`（$2 / $10）或 `claude-haiku-4-5`（$1 / $5）。

> Prompt caching 對這個 bot 幫助不大：快取預設只活 5 分鐘，而個人使用是一天零星幾則，幾乎每次都會 miss。只有連續來回好幾則時才有用。

建議到 https://console.anthropic.com/settings/limits 設一個每月支出上限，避免哪裡寫錯導致爆量。

---

## 規則層（省錢的關鍵）

`rules.py` 用正規表達式接住日常最常用的句型，完全不呼叫 Claude，回應也更快：

| 句型 | 例子 |
|---|---|
| 記帳 | `午餐 120`、`星巴克85`、`捷運 30`、`120 午餐`、`晚餐 250 元` |
| 查行程 | `今天有什麼安排`、`明天行程`、`這週有什麼安排`、`下週行程` |
| 查消費 | `這個月花多少`、`上個月花多少`、`餐飲花了多少` |
| 查預算 | `預算`、`還剩多少預算` |

接不住的才交給 Claude，例如 `下週二下午三點跟客戶開會`（要真的理解語意），或 `阿姨的東西 500`（猜不出分類）。

規則層刻意設計成**寧可放過、不可誤判**——記帳誤判會直接寫進 Notion。句子裡出現「開會、提醒、行程、預約、點」或日期符號（`/`、`:`、`月`、`日`）就一律不當成記帳。想調整分類判斷，改 `rules.py` 最上面的 `_CATEGORY_KEYWORDS`。

用 `python scripts/chat.py` 測試時，每則回覆會標示是「規則」還是「Claude」處理的，方便你調規則。
