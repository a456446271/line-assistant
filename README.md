...
| 分頁 | 能做什麼 |
|---|---|
| 待辦 | 新增、編輯、勾完成、刪除、看已完成（可還原）、「排」進行程、搜尋 |
| 行程 | 未來三週逐日列出（沒事的日子也列，只留一行日期）、新增、編輯、刪除、找空檔 |
| 記帳 | 本月總計與預算條、常用項目一鍵帶入品項、記一筆、編輯、刪除、搜尋過去一年 |
| 流程 | 列出、依分類篩選、搜尋、新增、編輯（含步驟）、**每一步可以打勾**（進度存手機，隔天歸零）|

記帳的常用快捷是從你自己的紀錄推的：過去半年**記過兩次以上**的項目才算數，
買一次沙發、買一款遊戲不會變成按鈕。歷史還不夠時用一組起手建議補位，
真實紀錄累積起來就會把它們擠掉。點下去只帶入品項與分類，**金額留空讓你自己填**
——同一個項目每次花的錢本來就不一樣，先填一個數字反而要多刪一次。

行程分頁把未來三週**每一天都列出來**，沒事的日子只留一行日期。只列有事的話
看不出哪幾天是空的，而「哪天有空」正是最常想知道的事。

待辦與流程的搜尋在前端就地過濾（清單本來就全在手上），記帳的搜尋要跨月份，
所以打後端，範圍是過去一年。

流程的打勾進度刻意存在瀏覽器的 localStorage、而不是 Notion：那是「今天做到哪」的
暫時狀態，不是資料。收班這種每天重跑一次的流程，昨天打的勾今天不該還留著，
所以進度綁日期，隔天自動歸零。換手機或清快取會消失，這是可接受的取捨。
