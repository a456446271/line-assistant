...
| 分頁 | 能做什麼 |
|---|---|
| 待辦 | 看清單、新增、勾完成、刪除、看已完成（可還原）、把待辦「排」進行程、搜尋 |
| 行程 | 看未來三週、新增（**不填時間就是整天行程**）、刪除、找空檔 |
| 記帳 | 本月總計與預算條、常用項目一鍵帶入、記一筆、刪除、搜尋過去一年 |
| 流程 | 列出、依分類篩選、搜尋、新增、**點開後每一步可以打勾**（進度存在手機裡，隔天自動歸零）|

記帳的常用快捷是從你自己的紀錄推的：過去半年**記過兩次以上**的項目才算數，
買一次沙發、買一款遊戲不會變成按鈕。金額用你最近一次付的，不是平均——
午餐從 100 漲到 140 之後，平均會給出一個你從來沒付過的數字。
歷史還不夠時用一組起手建議補位，真實紀錄累積起來就會把它們擠掉。
